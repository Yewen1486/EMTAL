...
class BasicBalancer(torch.nn.Module):
    ...
    def apply_decoder_scaling(decoders, weights):
        print(f'decoders: {decoders} weights: {weights}')
    ...
    def set_shared_grad(shared_params, grad_vec):
        # print(f'    shared_params: {shared_params} grad_vec: {grad_vec}')
        ...
        for p in shared_params:
            if p.grad is None:
                continue
            _offset = offset + p.grad.shape.numel()
            # print(f'        grad before view: {grad_vec[offset:_offset].shape}')
            # print(f'        grad shape before view: {grad_vec[offset:_offset].view_as(p.grad).shape} grad mean {grad_vec[offset:_offset].mean()}')
            p.grad.data = grad_vec[offset:_offset].view_as(p.grad)
            offset = _offset
