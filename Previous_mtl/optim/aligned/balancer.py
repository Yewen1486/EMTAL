...
print(f'starting aligned balancer')
@balancers.register("amtl1")
class AlignedMTLBalancer(basic_balancer.BasicBalancer):
    ...
    def step(self, losses,
        ...
        # print(f'grads shape: {grads.shape} {grads}')
        
        grads, weights, singulars = ProcrustesSolver.apply(grads.T.unsqueeze(0), self.scale_mode)
        
        # print(f'grads shape after procrustes: {grads.shape} {grads}')
        
        grad, weights = grads[0].sum(-1), weights.sum(-1)
        
        # print(f'grad shape after sum: {grad.shape} {grad}')

        self.set_shared_grad(shared_params, grad)
        
        try:
            self.set_losses({task_id: losses[task_id] * weights[i] for i, task_id in enumerate(losses) if losses[task_id]!=0})
        except:
            print(f'error in setting losses {losses}')
            # exit()

print(f'ending aligned balancer')
