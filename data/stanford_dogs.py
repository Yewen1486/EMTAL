...
class dogs(data.Dataset):
    ...
    def load_split(self):
        if 'train' in self.train:
            val = json.load(open(os.path.join(self.root, 'Images/val.json'), 'r'))
            split = scipy.io.loadmat(join(self.root, 'train_list.mat'))['annotation_list']
            labels = scipy.io.loadmat(join(self.root, 'train_list.mat'))['labels']
            if 'ssf' not in self.train:
                labels = [item[0]-1 for (item, item2) in zip(labels, split) if item2[0][0]+'.jpg' not in val.keys() ]
                split = [item[0][0] for item in split if item[0][0]+'.jpg' not in val.keys()]
            else:
            ...
            labels = scipy.io.loadmat(join(self.root, 'train_list.mat'))['labels']
            if 'ssf' not in self.train:
                labels = [item[0]-1 for (item, item2) in zip(labels, split) if item2[0][0]+'.jpg' in val.keys() ]
                split = [item[0][0] for item in split if item[0][0]+'.jpg' in val.keys()]
            else:
                split = [item[0][0] for item in split]
                labels = [item[0]-1 for item in labels]
            print('val')
            
        print(len(split), len(labels))
